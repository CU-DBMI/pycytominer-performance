...
sql_path = "SQ00014613.duckdb"
sql_url = "sqlite:///sample.sqlite"
...
def new_load_compartment(self, compartment):
    ...
    """
    compartment_query = f"select * from {compartment}" # nosec
    df = self.conn.execute(compartment_query).df()
    return df
...
def mem_profile_func():
    ...
    # setup alternative duckdb connection, overriding sqlite
    sc_p.conn = duckdb.connect(database=sql_path)
